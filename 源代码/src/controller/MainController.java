...
public class MainController {
	...
	public static SystemCore systemCore;

	public MainController() {
		super();

		// Initialize systemCore
		MainController.systemCore = new SystemCore();
	...
	private void configureMainView() {
		// 根据当前目录文件初始化view
		this.view = new MainView(MainController.systemCore.currentDir);
       ...
       //添加textfield监听
		this.view.addAddressTextFieldActionListener(this.addressTextFieldActionListener);
		// 添加前往按钮监听
		...
		// 显示当前路径
		this.view.addressTextField.setText(MainController.systemCore.getCurrentPath());
	}
	...
	private void showEditView(FCB fcb) {
		...
		EditView editView = new EditView(fcb,
				MainController.systemCore.readFile(fcb));
	...
	private MouseListener rightClickListener = new MouseListener() {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (e.getButton() == MouseEvent.BUTTON3) {
				boolean isRoot = (MainController.systemCore.currentDirFCB.fatherBlockId == -1);
	...
	private MouseListener documentIconPanelMouseListener = new MouseListener() {
		...
		public void mouseClicked(MouseEvent e) {
			...
			// 获取文件FCB
			FCB fcb = MainController.systemCore.getFCB(d.getFilename(),
					d.getType());

			// Double click
			if (e.getClickCount() == 2) {
				// 判断双击的类型
				if (d.getType() == ConstValue.FILE_TYPE.DIRECTORY) {
					// 双击文件夹
					// model进入下一级文件夹
					MainController.systemCore.enterDir(d.getFilename());
			...
			if (e.getButton() == MouseEvent.BUTTON3) {
				...
				if (d.getType() == ConstValue.FILE_TYPE.FILE) {
	...
	private ActionListener newFileActionListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (MainController.systemCore.createFile(filename)) {
				// 添加到model成功，即创建文件成功
				// 添加到view
				DocumentIconPanel d = new DocumentIconPanel(ConstValue.FILE_TYPE.FILE,
						filename);
	...
	private ActionListener newFolderActionListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (MainController.systemCore.createDir(filename)) {
				// 添加到model成功，即创建文件夹成功
				// 添加到view
				DocumentIconPanel d = new DocumentIconPanel(ConstValue.FILE_TYPE.DIRECTORY,filename);
	...
	private CustomActionListener openMenuActionListener = new CustomActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			if (this.fcb.type == ConstValue.FILE_TYPE.DIRECTORY) {
				// model进入下一级文件夹
				MainController.systemCore.enterDir(this.fcb.filename);
	...
	private CustomActionListener deleteMenuActionListener = new CustomActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (result == 0) {
				...
				if (this.fcb.type == ConstValue.FILE_TYPE.DIRECTORY) {
					// model删除文件夹
					MainController.systemCore.deleteDir(this.fcb.filename);
				} else {
					// model删除文件
					MainController.systemCore
	...
	private CustomActionListener renameMenuActionListener = new CustomActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// 更新文件FCB
			MainController.systemCore.updateFCB(this.fcb);

			// 更新当前目录的目录文件
			MainController.systemCore.updateFile(
					MainController.systemCore.currentDirFCB,
					gson.toJson(MainController.systemCore.currentDir));
	...
	private ActionListener formatMenuActionListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (result == 0) {
				// 确定格式化
				MainController.systemCore.format();
			}
	...
	private CustomActionListener getInfoMenuActionListener = new CustomActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			JOptionPane.showMessageDialog(MainController.this.view,
					MainController.systemCore.getFileInfo(this.fcb),
	...
	private ActionListener backButtonActionListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (MainController.systemCore.leaveDir()) {
				// 确认回到上一级目录
				// 重绘view
				MainController.this.refreshView();
			} else {
...
// address textfield listener
  private ActionListener addressTextFieldActionListener = new ActionListener()
  {
	...
	public void actionPerformed(ActionEvent e) {
		...
		// 获取地址栏地址
					...
					// 获取当前地址
					String currentPath = MainController.systemCore
							.getCurrentPath();
					...
					// 临时保存当前目录FCB和目录文件
					FCB fcb = MainController.systemCore.currentDirFCB;
					FCB[] dir = MainController.systemCore.currentDir;

					// 开始后退再前进
					boolean success = true;
					// 后退
					for (int j = 0; j < stepOut; j++) {
						MainController.systemCore.leaveDir();
					}

					// 前进
					for (int j = 0; j < stepIn; j++) {
						if (!MainController.systemCore.enterDir(pathArray[i++])) {
							success = false;
							break;
						}
					}
					...
					if (success) {
						...
						// 恢复
						MainController.systemCore.currentDirFCB = fcb;
						MainController.systemCore.currentDir = dir;
						MainController.systemCore.countFiles();
					}
	...
	private WindowListener mainWindowListener = new WindowListener() {
		...
		public void windowClosed(WindowEvent e) {
			...
			// 系统核心退出
			MainController.systemCore.exit();
		}
	...
	private void refreshView() {
		this.view.reloadContent(MainController.systemCore.currentDir);

		// 重新添加监听
		this.addListenerForEachDocumentIcon();
       
		// 更新当前路径
//		this.view.add(new JLabel(MainController.systemCore
//				.getCurrentPath()));
		this.view.pathLabel.setText(MainController.systemCore
				.getCurrentPath());
		this.view.addressTextField.setText(MainController.systemCore
				.getCurrentPath());
	}
...
public static void  main(String[]args) {
		MainController showMain = new MainController();
		showMain.showMainView();
	}
}
