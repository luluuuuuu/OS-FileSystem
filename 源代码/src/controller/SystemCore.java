...
// init system core 
	...
	public String getFileInfo(FCB fcb) {
		...
		result += ("Type: "
				+ (fcb.type == ConstValue.FILE_TYPE.FILE ? "file" : "directory") + "\n");
	...
	public boolean createDir(String filename) {
		if (!this.checkFilename(filename, ConstValue.FILE_TYPE.DIRECTORY)) {
			// 有重名
			return false;
		}
		...
		if (fcbBlockStart != 0 && fileDataBlockStart != 0) {
			FCB dirFcb = new FCB(filename, this.currentDirFCB.blockId,
					ConstValue.FILE_TYPE.DIRECTORY, ConstValue.FILE_MAX_BLOCKS,
					fileDataBlockStart, fcbBlockStart);
			FCB[] dir = new FCB[40];
	...
	public void deleteDir(String filename) {
		FCB fcb = this.getFCB(filename, ConstValue.FILE_TYPE.DIRECTORY);
	...
	private void recursiveDeleteDir(FCB fcb) {
		...
		FCB[] dir = gson.fromJson(new String(bf.array(), ConstValue.CHARSET),
				FCB[].class);
		...
		for (int i = 0; i < dir.length; i++) {
			...
			if (dir[i].type == ConstValue.FILE_TYPE.DIRECTORY) {
				// 删除文件夹
				// 递归调用
	...
	public void deleteFile(String filename) {
		FCB fcb = this.getFCB(filename, ConstValue.FILE_TYPE.FILE);
	...
	public boolean enterDir(String dirname) {
		FCB dirFCB = this.getFCB(dirname, ConstValue.FILE_TYPE.DIRECTORY);
		if (dirFCB == null) {
			// 所对应文件夹不存在
		...
		ByteBuffer bf = this.diskManager.io.read(
				this.currentDirFCB.dataStartBlockId, this.currentDirFCB.size);
		String json = new String(bf.array(), ConstValue.CHARSET);
	...
	public boolean leaveDir() {
		...
		FCB fcb = gson.fromJson(new String(bf.array(), ConstValue.CHARSET),
				FCB.class);
		...
		ByteBuffer dirBf = this.diskManager.io.read(fcb.dataStartBlockId,
				fcb.size);
		FCB[] dir = gson.fromJson(new String(dirBf.array(), ConstValue.CHARSET),
				FCB[].class);
	...
	private String recursiveGetPath(FCB fcb) {
		if (fcb.type != ConstValue.FILE_TYPE.DIRECTORY) {
			return null;
		}
		...
		String fatherFCBJson = new String(fatherBf.array(), ConstValue.CHARSET);
	 ...
	 * @return 找到的FCB
	 */
	public FCB getFCB(String filename, ConstValue.FILE_TYPE type) {
	 ...
	 * @return 布尔值
	 */
	private boolean checkFilename(String filename, ConstValue.FILE_TYPE type) {
		for (int i = 0; i < this.currentDir.length; i++) {
			if (this.currentDir[i] == null) {
				break;
			}
